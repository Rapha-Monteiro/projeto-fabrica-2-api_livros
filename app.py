...
# Bibliotecas
#jsonify = lista, dicionarios em arquivos json
# make_response = cria um objeto de resta HTTP completo, ele não apenas retorna os dados, mas também permite personalizar
# cabeçalhos e status code. Essa biblioteca vai transformar o JSON em resposta HTTP
# o jsonify funciona sem o make_response, usamos ele apenas para mais controle sobre resposta, ou seja, para obter outros
# status fora o 200

# 3. Intanciamos o Flask (criar um objeto a partir de uma classe)
# Uma instancia é o objeto real contruido a partir desse molde
app = Flask(__name__)
...
def update_livro(id):
    for livro in livros:
        if livro.get('id') == id:
            ...
            return make_response(jsonify(messagem=f'Livro id {id} atualizando com sucesso (PUT)',
                                         dados=livro))
    return make_response(jsonify(mensagem='Livro não encontrado'),404)
